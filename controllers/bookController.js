...
exports.book_create_get = (req, res, next) => {
  // Get all authors and genres, which we can use for adding our books.
  async.parallel(
    {
      authors(callback){
...
exports.book_delete_get = (req, res, next) => {
  async.parallel(
    ...
    (err, results)=>{
      if (err){
        debug(`Book delete GET Error: ${err}`)
        return next(err);
      }
      if (results.book == null){
        // No results.
...
exports.book_delete_post = (req, res) => {
  async.parallel(
    ...
    (err, results)=>{
      if(err){
        debug(`Book delete POST Error: ${err}`);
        return next(err);
      }
      ...
      // Book has no copies. Delete object.
      Book.findByIdAndRemove(req.body.bookid, (err)=>{
        if (err){
          return next(err);
        }
        // Successful- redirect to book list.
        res.redirect('/catalog/books')
      })
    }
  )
...
exports.book_update_get = (req, res, next) => {
  ...
  async.parallel(
    ...
    (err, results)=>{
      if(err){
        debug(`Book Update GET Error: ${err}`);
        return next(err);
      }
      if(results.book == null){
        // No results.
...
exports.book_update_post = [
  ...
  (req, res, next)=>{
    ...
    if (!errors.isEmpty()){
      // There are errors. Render form again with sanitized values/error values.
      ...
      async.parallel(
        ...
        (err, results)=>{
          if(err){
            debug(`Rerendeing update Error: ${err}`);
            return next(err);
          }
          // Mark our  selected genre as checked.
